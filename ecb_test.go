...
var testCases = /* const */ map[string]struct {
	key, in, out []byte
}{
	"AES-128-ECB": {
		[]byte{
...
func TestEncrypter(t *testing.T) {
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			e.CryptBlocks(data, data)
			if !bytes.Equal(data, tc.out) {
				t.Errorf("got %x, want %x", data, tc.out)
			}
		})
	}
}
...
func TestDecrypter(t *testing.T) {
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			d.CryptBlocks(data, data)
			if !bytes.Equal(data, tc.in) {
				t.Errorf("got %x, want %x", data, tc.in)
			}
		})
	}
}
